...
// Using React.FC (FunctionComponent) to define the component's type
const SetPasswordModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({
  isOpen,
  onClose,
}) => {
  // TypeScript infers the type of state variables based on initial values
...
const SetPasswordForm = () => {
  ...
  return (
    <div className='min-h-screen bg-slate-100 p-8 flex flex-col items-center justify-center'>
      <SetPasswordModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
    </div>
  );
};
